namespace Win32
{
    ...
    {
        public static BitmapFont<ConsoleChar> FromConsoleImage(ConsoleImage image, int charWidth, int charHeight)
            => new(image.AsSpan(), image.Width, image.Height, charWidth, charHeight);

        public static BitmapFont<TPixel> FromConsoleImage<TPixel>(ConsoleImage image, int charWidth, int charHeight, Func<ConsoleChar, TPixel> converter)
