...
public static partial class User32
{
    ...
    /// </returns>
    /// <remarks>
    /// The <see cref="KillTimer"/> function does not remove
    ...
    /// replaced by the new timer. When <see cref="SetTimer"/> replaces a timer,
    ...
    /// The <see cref="MSG.hwnd"/> member of the message's <see cref="MSG"/> structure contains the
    ...
    /// calling <see cref="BlockInput"/> with <paramref name="fBlockIt"/> set to <c>FALSE</c>. In this case,
    ...
    /// the thread to a GUI thread, <see cref="IsGUIThread"/> returns <c>ERROR_NOT_ENOUGH_MEMORY</c>.
    /// </para>
    /// </returns>
    [DllImport("User32.dll", SetLastError = true)]
    ...
    public static extern unsafe BOOL PeekMessageW(
          ...
          [In] uint wMsgFilterMin,
          [In] uint wMsgFilterMax,
          [In] Forms.PeekMessageFlags wRemoveMsg
    );
    ...
    public static extern unsafe BOOL EndPaint(
          [In] HWND hWnd,
          [In] Gdi32.PaintStruct* lpPaint
    );

    [DllImport("User32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
    public static extern unsafe HDC BeginPaint(
          [In] HWND hWnd,
          [Out] Gdi32.PaintStruct* lpPaint
    );
